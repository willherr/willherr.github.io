...
**Currently a hobby/legacy page, not yet the front-company hub it's slated to become** — see the open backlog below.

**Non-obvious mechanism worth knowing**: `/WillsTools/` and `/Angular-RouteReuseStrategy/` on `will-i-am.dev` are real, live pages, not local files in this repo — GitHub Pages serves a *project* repo (one with its own Pages enabled, no custom domain of its own) at `<user's custom domain>/<repo name>/` automatically when the user/org site (this repo) owns that custom domain. Both those repos have their own Pages source (`main` branch, `/docs`), confirmed live and `built` — archiving `Angular-RouteReuseStrategy` (2026-09-10) didn't break this, Pages keeps serving an archived repo's last build. Don't assume a path under this domain that isn't a file in *this* repo is broken — check the linked repo's own Pages config before calling it stale.

Known real drift as of 2026-09-10, not yet cleaned up:
- `index.html`'s nav still links to `/CassandrasCookbook`, which is genuinely dead (`404`, confirmed) — that repo's Blazor-era GitHub Pages was never carried over to the Flutter rewrite, and the new app isn't deployed anywhere public yet (eventual subdomain `cookbook.will-i-am.dev`, see `willherr/CassandrasCookbook`'s own `CLAUDE.md`). `README.md`'s equivalent entry was fixed 2026-09-10; `index.html`'s nav link itself is still broken and hasn't been touched.
